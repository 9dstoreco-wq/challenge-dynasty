...
export default function SocialPostCard({ post, currentUserId }: { post:any; currentUserId?:string|null }) {
  ...
  return <article className="rounded-3xl border border-white/10 bg-[#141B2D] overflow-hidden">
    <div className="p-5 flex items-start justify-between"><div className="flex items-center gap-3"><div className="h-10 w-10 rounded-full bg-gradient-to-br from-cyan-300 to-blue-600"/><div><a href={`/u/${post.username}`} className="font-bold hover:text-[#00F0FF]">{post.full_name}</a><div className="text-xs text-white/40">{new Date(post.created_at).toLocaleString('es-CO')}</div></div></div>{currentUserId && post.user_id!==currentUserId && <button onClick={follow} disabled={busy} className="text-xs rounded-full border border-white/10 px-3 py-1.5 text-white/60">{following?'Siguiendo':'Seguir'}</button>}</div>
    <div className="px-5 pb-5"><div className="text-xs tracking-[.2em] text-[#00F0FF]">{post.type==='AUTO_MATCH_RESULT'?'🏆 RESULTADO OFICIAL':'POST'}</div>{post.title && <h3 className="text-xl font-black mt-2">{post.title}</h3>}<p className="mt-3 text-white/80 whitespace-pre-wrap">{post.content}</p>
      {match && <div className="mt-4 rounded-2xl bg-white/5 p-4"><div className="text-xs text-white/40">RESULTADO</div><div className="text-lg font-black mt-1">{match.winner_name} {match.score_set1 && `· ${match.score_set1}`} {match.score_set2 && `· ${match.score_set2}`} {match.score_set3 && `· ${match.score_set3}`} <span className="text-white/30">vs</span> {match.loser_name}</div><a href={`/challenge/${match.challenge_id}`} className="inline-flex mt-3 rounded-xl bg-[#00F0FF] text-black px-3 py-2 text-xs font-black"><Swords size={14}/> REVANCHA</a></div>}
