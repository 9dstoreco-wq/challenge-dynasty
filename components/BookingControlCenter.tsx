...
export default function BookingControlCenter({bookables,bookings}:{bookables:Bookable[];bookings:Booking[]}){
 ...
 return <div className="space-y-6">
  <section className="rounded-3xl border border-white/10 bg-[#161616] p-6">
   <div className="flex items-start justify-between gap-4"><div><div className="text-xs tracking-[.25em] text-[#D4AF37] font-black">BOOKING ENGINE</div><h2 className="text-2xl font-black mt-1">RESERVA ATÓMICA</h2><p className="text-sm text-white/45 mt-2 max-w-2xl">Disponibilidad, capacidad, conflictos y pago quedan validados por el backend antes de confirmar.</p></div><ShieldCheck className="text-[#D4AF37]"/></div>
   {bookables.length===0 ? <div className="mt-6 rounded-2xl border border-dashed border-white/10 p-8 text-white/45">No hay recursos reservables activos todavía.</div> : <>
    <div className="grid md:grid-cols-[1.4fr_.8fr_.5fr] gap-3 mt-6">
      <select value={bookableId} onChange={e=>setBookableId(e.target.value)} className="bg-[#0A0A0C] border border-white/10 rounded-xl p-3 text-sm">{bookables.map(x=><option key={x.id} value={x.id}>{x.title} · {x.activity_name||'Servicio'} · {Number(x.price).toLocaleString('es-CO')} {x.currency_code}</option>)}</select>
      <input type="datetime-local" value={starts} onChange={e=>setStarts(e.target.value)} className="bg-[#0A0A0C] border border-white/10 rounded-xl p-3 text-sm" aria-label="Inicio de reserva"/>
      <input type="number" min="1" max={selected?.capacity||1} value={quantity} onChange={e=>setQuantity(Math.max(1,Math.min(selected?.capacity||1,Number(e.target.value)||1)))} className="bg-[#0A0A0C] border border-white/10 rounded-xl p-3 text-sm" aria-label="Cantidad"/>
    </div>
    {selected&&<div className="grid md:grid-cols-4 gap-3 mt-4 text-sm"><div className="rounded-xl bg-black/15 p-3"><Clock3 size={15} className="text-[#D4AF37]"/><div className="text-white/40 mt-1">Duración</div><strong>{selected.duration_minutes} min</strong></div><div className="rounded-xl bg-black/15 p-3"><CalendarDays size={15} className="text-[#D4AF37]"/><div className="text-white/40 mt-1">Capacidad</div><strong>{selected.capacity}</strong></div><div className="rounded-xl bg-black/15 p-3"><CreditCard size={15} className="text-[#D4AF37]"/><div className="text-white/40 mt-1">Total</div><strong>{(Number(selected.price)*quantity).toLocaleString('es-CO')} {selected.currency_code}</strong></div><div className="rounded-xl bg-black/15 p-3"><div className="text-white/40">Termina</div><strong>{new Date(end).toLocaleString('es-CO')}</strong></div></div>}
    <button disabled={busy||!selected} onClick={reserve} className="mt-5 w-full md:w-auto rounded-xl bg-[#D4AF37] text-black font-black px-6 py-3 inline-flex items-center justify-center gap-2">{busy?<Loader2 size={16} className="animate-spin"/>:<CalendarDays size={16}/>} {busy?'PROCESANDO…':'CONFIRMAR RESERVA'}</button>
   </>}
  ...
  <section className="rounded-3xl border border-white/10 bg-[#161616] p-6"><div className="flex items-center justify-between"><div><div className="text-xs tracking-[.2em] text-[#D4AF37] font-black">MY BOOKINGS</div><h2 className="text-2xl font-black mt-1">Agenda y control</h2></div><span className="text-xs text-white/40">{bookings.length} reservas</span></div>{bookings.length===0?<div className="mt-5 rounded-2xl border border-dashed border-white/10 p-8 text-white/45">Aún no tienes reservas. Cuando haya datos, aquí aparecerán estado, pago y acciones.</div>:<div className="mt-5 space-y-3">{bookings.map(b=><article key={b.id} className="rounded-2xl border border-white/10 bg-black/10 p-4"><div className="flex flex-wrap items-start justify-between gap-4"><div><div className="text-xs text-[#D4AF37] uppercase tracking-widest font-black">{b.status}</div><div className="font-black mt-1">{new Date(b.starts_at).toLocaleString('es-CO')}</div><div className="text-xs text-white/40 mt-1">Pago: {b.payment_status} · Cantidad: {b.quantity}</div></div><div className="text-right"><div className="font-black">{Number(b.amount).toLocaleString('es-CO')} {b.currency_code}</div><button disabled={busy||b.status==='cancelled'} onClick={()=>cancel(b.id)} className="mt-2 text-xs font-black text-red-200 inline-flex items-center gap-1"><XCircle size={14}/> Cancelar</button></div></div></article>)}</div>}</section>
 </div>
}
