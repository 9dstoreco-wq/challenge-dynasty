...
export default function MatchResultForm({ challengeId, matchId, resultId, creatorId, opponentId, creatorName, opponentName, currentUserId, winnerId, submittedBy, resultStatus, scoreSet1='', scoreSet2='', scoreSet3='' }: Props) {
  ...
  return <section className="mt-8 rounded-3xl border border-white/10 bg-white/[.03] p-5"><div className="text-xs tracking-[.25em] text-[#FFD700] font-black">⚡ RESULTADO DEL PARTIDO</div><h2 className="text-2xl font-black mt-2">Jugar → registrar → confirmar</h2><p className="text-sm text-white/45 mt-1">El otro jugador revisará el resultado.</p><form onSubmit={submit} className="mt-5 space-y-4"><div><label className="text-xs text-white/45">GANADOR</label><div className="grid sm:grid-cols-2 gap-2 mt-2"><button type="button" onClick={()=>setWinner(creatorId)} className={`rounded-xl px-4 py-3 border font-black ${winner===creatorId?'border-[#00F0FF] bg-[#00F0FF]/10':'border-white/10 bg-white/5'}`}>{creatorName ?? 'Creador'}</button><button type="button" onClick={()=>opponentId&&setWinner(opponentId)} disabled={!opponentId} className={`rounded-xl px-4 py-3 border font-black ${winner===opponentId?'border-[#00F0FF] bg-[#00F0FF]/10':'border-white/10 bg-white/5'}`}>{opponentName ?? 'Rival'}</button></div></div><div className="grid sm:grid-cols-3 gap-3"><input value={set1} onChange={e=>setSet1(e.target.value)} placeholder="Set 1 · 6-4" required className="rounded-xl bg-white/5 border border-white/10 px-4 py-3"/><input value={set2} onChange={e=>setSet2(e.target.value)} placeholder="Set 2 · 6-3" required className="rounded-xl bg-white/5 border border-white/10 px-4 py-3"/><input value={set3} onChange={e=>setSet3(e.target.value)} placeholder="Set 3 · opcional" className="rounded-xl bg-white/5 border border-white/10 px-4 py-3"/></div>{error&&<div className="rounded-xl bg-red-500/10 border border-red-400/20 p-3 text-sm text-red-200">{error}</div>}<button disabled={busy||!winner||!opponentId} className="w-full rounded-2xl bg-[#00F0FF] text-black py-4 font-black">{busy?'GUARDANDO...':'GUARDAR RESULTADO'}</button></form>{resultId && <div className="mt-3 text-center text-xs text-white/30">Resultado registrado: {resultId}</div>}</section>
}
