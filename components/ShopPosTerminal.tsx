...
export default function ShopPosTerminal({products, locations, registers}:{products:Product[];locations:Location[];registers:Register[]}){
  ...
  async function openRegister(){
    ...
    setMessage(error?toSafeMessage(error,'shopPos.openRegister'):'Caja abierta correctamente.')
    setBusy(false)
  }
  async function saveCustomer(){
    ...
    setMessage(error?toSafeMessage(error,'shopPos.saveCustomer'):'Cliente guardado.')
    setBusy(false)
  }
  async function sell(){
    ...
    if(!error){setResult(data);setCart([]);setMessage('Venta registrada correctamente.')} else setMessage(toSafeMessage(error,'shopPos.sell'))
    setBusy(false)
  }
  ...
  return <div className="grid xl:grid-cols-[1fr_430px] gap-6">
    <section className="rounded-3xl border border-white/10 bg-[#161616] p-5">
      <div className="flex flex-wrap gap-3 items-center mb-5">
        <select value={locationId} onChange={e=>{setLocationId(e.target.value);setRegisterId(registers.find(r=>r.location_id===e.target.value)?.id||'')}} className="bg-[#0A0A0C] border border-white/10 rounded-xl px-3 py-2 text-sm"><option value="">Ubicación</option>{locations.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select>
        <select value={registerId} onChange={e=>setRegisterId(e.target.value)} className="bg-[#0A0A0C] border border-white/10 rounded-xl px-3 py-2 text-sm"><option value="">Caja</option>{registers.filter(r=>r.location_id===locationId).map(r=><option key={r.id} value={r.id}>{r.name} · {r.status}</option>)}</select>
      ...
      {products.length===0?<div className="rounded-2xl border border-dashed border-white/10 p-8 text-center text-white/40">No hay productos cargados todavía. La terminal está conectada al catálogo real.</div>:<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">{filtered.map(p=><div key={p.id} className="rounded-2xl bg-white/[.03] border border-white/10 p-4"><div className="font-black text-sm">{p.title}</div><div className="space-y-2 mt-3">{(p.variants.length?p.variants:[{id:`base-${p.id}`,sku:'',title:'',price:p.base_price}]).map((v)=><button key={v.id} onClick={()=>v.id.startsWith('base-')?setMessage('Este producto aún no tiene una variante operativa para POS.'):add(p,v)} className="w-full text-left rounded-xl border border-white/10 hover:border-[#D4AF37]/50 p-3"><div className="flex justify-between gap-2"><span>{v.title||v.sku||'Variante'}</span><b>{Number(v.price??p.base_price).toLocaleString('es-CO')}</b></div><div className="text-[10px] text-white/30 mt-1">{v.sku||'Sin SKU'}</div></button>)}</div></div>)}</div>}
    </section>
    <aside className="space-y-5">
      <section className="rounded-3xl border border-white/10 bg-[#161616] p-5"><div className="flex items-center justify-between"><h2 className="font-black text-lg">Venta</h2><Receipt size={18} className="text-[#D4AF37]"/></div>
        ...
        <div className="grid grid-cols-2 gap-2 mt-4">{([['cash','Efectivo',Banknote],['card','Tarjeta',CreditCard]] as const).map(([v,label,Icon])=><button key={v} onClick={()=>setPayment(v)} className={`rounded-xl p-3 border ${payment===v?'border-[#D4AF37] bg-[#D4AF37]/10':'border-white/10'}`}><Icon size={17} className="mx-auto mb-1"/><span className="text-xs font-bold">{label}</span></button>)}</div>
        <textarea value={notes} onChange={e=>setNotes(e.target.value)} className="mt-3 w-full bg-[#0A0A0C] border border-white/10 rounded-xl p-3 text-sm" placeholder="Notas de la venta…"/>
        <button disabled={busy||!cart.length} onClick={sell} className="mt-3 w-full rounded-xl bg-[#D4AF37] text-black font-black py-3 disabled:opacity-40">{busy?'PROCESANDO…':'COBRAR VENTA'}</button>
      </section>
      <section className="rounded-3xl border border-white/10 bg-[#161616] p-5"><div className="flex items-center gap-2 font-black"><UserPlus size={17}/> Cliente</div><div className="grid grid-cols-2 gap-2 mt-3"><input value={fullName} onChange={e=>setFullName(e.target.value)} className="col-span-2 bg-[#0A0A0C] border border-white/10 rounded-xl p-3 text-sm" placeholder="Nombre completo"/><input value={phone} onChange={e=>setPhone(e.target.value)} className="bg-[#0A0A0C] border border-white/10 rounded-xl p-3 text-sm" placeholder="Teléfono"/><input value={email} onChange={e=>setEmail(e.target.value)} className="bg-[#0A0A0C] border border-white/10 rounded-xl p-3 text-sm" placeholder="Email"/></div><button disabled={busy||!fullName.trim()} onClick={saveCustomer} className="mt-3 w-full rounded-xl border border-white/10 py-3 text-sm font-black">{customerId?'CLIENTE ASIGNADO':'GUARDAR / ASIGNAR CLIENTE'}</button></section>
      <section className="rounded-3xl border border-white/10 bg-[#161616] p-5"><div className="font-black flex items-center gap-2"><LockOpen size={17}/> Apertura de caja</div><div className="flex gap-2 mt-3"><input value={openingFloat} onChange={e=>setOpeningFloat(e.target.value)} className="flex-1 bg-[#0A0A0C] border border-white/10 rounded-xl p-3 text-sm" type="number" min="0" placeholder="Fondo inicial"/><button disabled={busy||!registerId} onClick={openRegister} className="rounded-xl bg-[#D4AF37] text-black px-4 font-black">ABRIR</button></div></section>
      {message&&<div className="rounded-2xl border border-white/10 bg-white/[.03] p-4 text-sm">{message}</div>}
      {result&&<div className="rounded-2xl border border-[#D4AF37]/20 bg-[#D4AF37]/5 p-4 text-sm"><div className="font-black">Comprobante generado</div><pre className="text-[10px] text-white/50 mt-2 whitespace-pre-wrap">{JSON.stringify(result,null,2)}</pre></div>}
    </aside>
  </div>
}
