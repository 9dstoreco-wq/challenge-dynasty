...
import { COUNTRIES, countryLabel, countryName } from '@/lib/countries'
...
export default function MarketplaceClient({ listings }: { listings: Listing[] }) {
  ...
  async function reserve() {
    ...
    try {
      ...
      if (!userData.user) throw new Error(t('debesIniciarSesion'))
      const { data, error: rpcError } = await supabase.rpc('create_marketplace_order', {
        p_items: [{ listing_id: selected.id, quantity: qty }],
        p_delivery_amount: 0,
      })
      if (rpcError) throw rpcError
      setMessage(`${t('ordenReservada')}: ${String(data).slice(0, 8)}…`)
      setSelected(null)
    } catch (e) {
      setError(toSafeMessage(e, 'marketplace.reserve', t('noSePudoCrearOrden')))
    } finally {
      setBusy(false)
    }
  }
    ...
    <>
      {availableCountries.length > 1 && (
        <div className="mb-6 flex items-center gap-3 flex-wrap">
          <span className="text-xs uppercase tracking-widest text-white/40 font-black">{t('pais')}</span>
          <button onClick={() => setCountryFilter('all')} className={`rounded-full px-4 py-2 text-sm font-black border ${countryFilter === 'all' ? 'bg-[#D4AF37] text-black border-[#D4AF37]' : 'border-white/10 text-white/60'}`}>{t('todos')}</button>
          {availableCountries.map((c) => (
            <button key={c.code} onClick={() => setCountryFilter(c.code)} className={`rounded-full px-4 py-2 text-sm font-black border ${countryFilter === c.code ? 'bg-[#D4AF37] text-black border-[#D4AF37]' : 'border-white/10 text-white/60'}`}>{c.flag} {countryName(c.code, locale)}</button>
          ))}
        </div>
      )}
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.map((x) => (
          <article key={x.id} className="rounded-3xl border border-white/10 bg-[#161616] p-6 flex flex-col">
            ...
            <p className="text-white/45 text-sm mt-2 flex-1">{x.description || 'Marketplace'}</p>
            <div className="mt-4 text-lg font-black">{x.price != null ? `${Number(x.price).toLocaleString('es-CO')} ${x.currency_code || 'COP'}` : t('precioAConsultar')}</div>
            <div className="text-xs text-white/35 mt-2 flex items-center gap-2 flex-wrap">
              <span>{x.city || t('ubicacionNoEspecificada')}</span>
              <span className="text-white/20">·</span>
              <span>{countryLabel(x.country_code, locale)}</span>
            </div>
            <button onClick={() => { setSelected(x); setQty(1); setError(null); setMessage(null) }} className="mt-5 rounded-2xl bg-white text-black font-black px-4 py-3 inline-flex items-center justify-center gap-2">
              <ShoppingCart size={17} /> {t('reservarComprar')}
            </button>
          </article>
        ))}
      </div>
      ...
      {filtered.length === 0 && (
        <div className="rounded-3xl border border-dashed border-white/15 bg-white/[.02] p-8 text-center text-white/45">{t('noPublicacionesPais')}</div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#111827] p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-xs uppercase tracking-widest text-[#D4AF37] font-black">{t('nuevaOrden')}</div>
            ...
            <div className="mt-6">
              <label className="text-xs uppercase tracking-widest text-white/45 font-black">{t('cantidad')}</label>
            ...
            <button disabled={busy} onClick={reserve} className="mt-5 w-full rounded-2xl bg-[#D4AF37] text-black font-black px-5 py-3 flex items-center justify-center gap-2 disabled:opacity-60">
              {busy ? <Loader2 size={17} className="animate-spin" /> : <ShoppingCart size={17} />} {busy ? t('procesando') : t('crearOrdenReservada')}
            </button>
            <p className="text-xs text-white/35 mt-3 text-center">{t('footNote')}</p>
          </div>
        </div>
      )}
    </>
