...
export default function PartnerInviteButton({sportId,recipientId}:{sportId:string;recipientId:string}){
 ...
 return <button disabled={busy||sent} onClick={invite} className="flex-1 rounded-xl bg-[#D4AF37] text-black py-3 text-xs font-black">{busy?'ENVIANDO...':sent?'✓ ENVIADA':'INVITAR'}</button>
}
