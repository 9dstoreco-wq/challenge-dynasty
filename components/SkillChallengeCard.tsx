...
export default function SkillChallengeCard({title,user,rank,votes,difficulty,points,category,accent}:{title:string;user:string;rank:string;votes:number;difficulty:string;points:number;category:string;accent:string}){
  ...
  return <article className="rounded-3xl border border-white/10 bg-[#161616] overflow-hidden hover:border-gold-300/30 transition group">
    <div className={`aspect-[16/9] bg-gradient-to-br ${accent} relative flex items-center justify-center`}>
      ...
      <div className="absolute top-4 right-4 rounded-full bg-black/45 border border-white/10 px-3 py-1 text-[10px] font-black tracking-wider flex items-center gap-1"><Flame size={12} className="text-[#FF3D00]"/> {t('trending')}</div>
      <button aria-label={t('watchAria')} className="h-16 w-16 rounded-full bg-black/45 backdrop-blur border border-white/20 flex items-center justify-center group-hover:scale-110 transition"><Play fill="white" size={24}/></button>
    ...
    <div className="p-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="text-right"><div className="text-[10px] text-white/35">{t('votesLabelShort')}</div><div className="font-black text-[#D4AF37]">{votes}</div></div>
      </div>
      <div className="grid grid-cols-3 gap-2 mt-5">
        <button className="rounded-xl bg-white/5 py-3 text-xs font-black flex items-center justify-center gap-1"><Heart size={14}/> {t('voteBtn')}</button>
        <button className="rounded-xl bg-white/5 py-3 text-xs font-black flex items-center justify-center gap-1"><MessageCircle size={14}/> {t('commentBtn')}</button>
        <button className="rounded-xl bg-[#D4AF37] text-black py-3 text-xs font-black flex items-center justify-center gap-1"><Swords size={14}/> {t('beatBtn')}</button>
      </div>
    </div>
  </article>
}
