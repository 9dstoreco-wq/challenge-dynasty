...
export default function SkillChallengeCard({title,user,rank,votes,difficulty,points,category,accent}:{title:string;user:string;rank:string;votes:number;difficulty:string;points:number;category:string;accent:string}){
  return <article className="rounded-3xl border border-white/10 bg-[#141B2D] overflow-hidden hover:border-cyan-300/30 transition group">
    <div className={`aspect-[16/9] bg-gradient-to-br ${accent} relative flex items-center justify-center`}>
      ...
      <div className="absolute bottom-4 right-4 rounded-full bg-black/50 border border-white/10 px-3 py-1 text-xs font-black flex items-center gap-1"><Zap size={12} className="text-[#00F0FF]"/> +{points}</div>
    </div>
    <div className="p-5">
      <div className="flex items-start justify-between gap-4">
        <div><h3 className="text-xl font-black">{title}</h3><p className="text-sm text-white/45 mt-1">{user} · {rank}</p></div>
        <div className="text-right"><div className="text-[10px] text-white/35">VOTOS</div><div className="font-black text-[#FFD700]">{votes}</div></div>
      </div>
      <div className="grid grid-cols-3 gap-2 mt-5">
        ...
        <button className="rounded-xl bg-[#00F0FF] text-black py-3 text-xs font-black flex items-center justify-center gap-1"><Swords size={14}/> SUPERAR</button>
      </div>
    </div>
  </article>
}
