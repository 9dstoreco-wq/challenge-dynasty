...
export default async function TrickDetail({params}:{params:Promise<{id:string}>}){
  ...
  return <div className="min-h-screen bg-[#0A0A0C] grid-bg text-white"><Sidebar/><main className="lg:pl-64 pb-20 lg:pb-0"><div className="max-w-5xl mx-auto px-4 md:px-8 py-8 space-y-6"><section className="rounded-3xl border border-[#D4AF37]/20 bg-gradient-to-br from-[#1B1A12] via-[#161616] to-[#0A0A0C] p-6 md:p-10"><div className="flex flex-wrap items-center gap-2 text-xs font-black"><span className="rounded-full bg-[#D4AF37]/10 text-[#D4AF37] px-3 py-1">{challenge.category}</span><span className="rounded-full bg-white/5 px-3 py-1">{challenge.difficulty}</span><span className="rounded-full bg-white/5 px-3 py-1">+{challenge.points} XP</span></div><h1 className="text-4xl md:text-5xl font-display font-black tracking-wide mt-5">{challenge.title}</h1><p className="text-white/55 mt-4 max-w-3xl">{challenge.description??t('defaultDescription')}</p><div className="text-xs text-white/35 mt-5">{t('createdBy',{name:creator?.display_name??creator?.username??t('defaultCreator'),count:submissions?.length??0})}</div></section><SkillSubmissionForm challengeId={challenge.id}/><section className="space-y-4"><h2 className="text-2xl font-black">{t('communityProofsTitle')}</h2>{(submissions??[]).map((s)=>{const u=bySubmitter.get(s.user_id);return <article key={s.id} className="rounded-3xl border border-white/10 bg-[#161616] p-5"><div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4"><div><div className="font-black text-lg">{u?.display_name??u?.username??t('defaultPlayer')}</div><div className="text-xs text-white/35 mt-1">{new Date(s.created_at).toLocaleString('es-CO')}</div></div><div className="flex items-center gap-2"><span className="text-xs text-white/35">{t('votesLabel',{count:s.votes})}</span><span className="text-xs font-black text-[#00E676]">+{s.skill_points_awarded??0} XP</span><VoteSkillButton submissionId={s.id}/></div></div>{s.caption&&<p className="text-white/55 mt-4">{s.caption}</p>}<a href={s.video_url} target="_blank" rel="noreferrer" className="inline-flex mt-4 rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-xs font-black">{t('viewProof')}</a></article>})}{(submissions??[]).length===0&&<div className="rounded-3xl border border-white/10 bg-[#161616] p-8 text-center text-white/45">{t('emptySubmissions')}</div>}</section></div></main><BottomNav/></div>
}
