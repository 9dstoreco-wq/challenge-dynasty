...
export default async function ShopAdminPage(){
  ...
  return <main className="min-h-screen bg-[#0B0F19] text-white grid-bg"><Sidebar/><section className="lg:pl-64 p-6 lg:p-12 max-w-[1500px] mx-auto">
    ...
    <div className="grid md:grid-cols-4 gap-4 mt-8">{[['ONLINE',online],['FÍSICO',physical],['VENTAS',revenue.toLocaleString('es-CO')],['ALERTAS',alerts.length]].map(([k,v])=><div key={k} className="rounded-3xl border border-white/10 bg-[#141B2D] p-6"><div className="text-xs text-white/40 tracking-widest">{k}</div><div className="text-3xl font-black mt-2">{v}</div></div>)}</div>
    <div className="grid lg:grid-cols-2 gap-5 mt-6"><div className="rounded-3xl border border-white/10 bg-[#141B2D] p-6"><h2 className="font-black text-xl">Inventario</h2><p className="text-sm text-white/40 mt-1">Físico y online, por ubicación y variante.</p><div className="mt-5 space-y-2 max-h-96 overflow-auto">{inventory.slice(0,30).map((x,i:number)=><div key={i} className="flex justify-between rounded-xl bg-white/[.03] p-3"><span className="truncate pr-3">{x.product_title || x.variant_title || 'Producto'}</span><b>{x.available_quantity ?? 0}</b></div>)}</div></div>
