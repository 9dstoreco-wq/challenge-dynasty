...
export default async function ShopPosPage(){
  ...
  const t = await getTranslations('ShopPos')
  ...
  return <main className="min-h-screen bg-[#0A0A0C] text-white grid-bg"><Sidebar/><section className="lg:pl-64 p-6 lg:p-12 max-w-[1700px] mx-auto"><div className="mb-8"><PageHero><div className="text-xs tracking-[.3em] text-[#D4AF37] font-black">{t('tag')}</div><h1 className="text-4xl md:text-6xl font-display font-black tracking-wide mt-2">{t('title')}</h1><p className="text-white/50 mt-3 max-w-3xl">{t('subtitle')}</p></PageHero></div><ShopPosTerminal products={grouped} locations={locations ?? []} registers={registers ?? []}/></section></main>
}
