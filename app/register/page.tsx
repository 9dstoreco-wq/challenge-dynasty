...
export default function RegisterPage() {
  ...
  async function submit(e: React.FormEvent){e.preventDefault();setMessage('');setLoading(true);const {error}=await supabase.auth.signUp({email,password,options:{data:{full_name:name,username}}}); if(error)setMessage(error.message); else setMessage('Cuenta creada. Revisa tu correo si la confirmación está activada.');setLoading(false)}
