...
export default function OnboardingPage(){
  ...
  async function save(){const supabase=createClient();if(!sport){setMessage('Selecciona un deporte.');return} setSaving(true); setMessage(''); const {data:{user}}=await supabase.auth.getUser(); if(!user){setMessage('Inicia sesión primero.');setSaving(false);return}
    const meta:any=user.user_metadata||{}; const fallbackName=meta.full_name||meta.username||(user.email?user.email.split('@')[0]:'Jugador Dynasty')
    const {error:profileError}=await supabase.from('profiles').upsert({id:user.id,display_name:fallbackName,username:meta.username||null},{onConflict:'id',ignoreDuplicates:true})
    if(profileError&&(profileError as any).code==='23505'){await supabase.from('profiles').upsert({id:user.id,display_name:fallbackName},{onConflict:'id',ignoreDuplicates:true})}
