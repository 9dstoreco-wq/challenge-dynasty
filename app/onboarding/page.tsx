...
export default function OnboardingPage(){
  ...
  async function save(){const supabase=createClient();if(!sport){setMessage(t('selectSport'));return} setSaving(true); setMessage(''); const {data:{user}}=await supabase.auth.getUser(); if(!user){setMessage(t('loginFirst'));setSaving(false);return}
    ...
    const fallbackName=metaFullName||metaUsername||(user.email?user.email.split('@')[0]:t('defaultName'))
    ...
    const {data}=await supabase.from('sports').select('id').eq('slug',sport).single(); if(!data){setMessage(t('sportUnavailable'));setSaving(false);return} const {error}=await supabase.from('player_sports').upsert({profile_id:user.id,sport_id:data.id,is_primary:true,is_discoverable:true,relationship:'primary',skill_level:null},{onConflict:'profile_id,sport_id'}); if(error)setMessage(toSafeMessage(error,'onboarding.saveSport')); else setMessage(t('done')); setSaving(false)}
  return <main className="min-h-screen bg-[#0A0A0C] text-white grid place-items-center p-6"><section className="w-full max-w-3xl rounded-3xl border border-[#D4AF37]/20 bg-gradient-to-br from-[#1B1A12] via-[#161616] to-[#0A0A0C] p-7"><div className="text-xs tracking-[.3em] text-[#D4AF37] font-black">{t('step')}</div><h1 className="text-3xl md:text-5xl font-display font-black tracking-wide mt-2">{t('title')}</h1><p className="text-white/50 mt-2">{t('subtitle')}</p><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-7">{loading ? <div className="sm:col-span-2 lg:col-span-3 p-6 text-white/45">{t('loading')}</div> : sports.map(s=><button key={s.id} onClick={()=>setSport(s.slug)} className={`rounded-2xl p-4 text-left border ${sport===s.slug?'border-[#D4AF37] bg-[#D4AF37]/10':'border-white/10 bg-white/5'}`}>{s.icon||'🏆'} {s.name}</button>)}</div>{message&&<p className="text-white/60 mt-4">{message}</p>}<div className="flex flex-wrap gap-3 mt-6"><button disabled={saving||loading} onClick={save} className="rounded-xl bg-[#D4AF37] text-black px-5 py-3 font-black">{saving?t('saving'):t('saveSport')}</button><Link href="/" className="rounded-xl bg-white/5 border border-white/10 px-5 py-3 font-bold">{t('goToChallenge')}</Link></div></section></main>
}
