...
export default async function SocialPage(){
  ...
  const {data:posts}=await supabase.from('social_posts').select('id,author_profile_id,post_type,body,created_at,profiles!social_posts_author_profile_id_fkey(username,display_name)').order('created_at',{ascending:false}).limit(40)
  ...
  const liked=new Set((likes??[]).map((x:any)=>x.post_id))
