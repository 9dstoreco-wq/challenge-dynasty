...
export default async function Business(){
  ...
  return <div className="min-h-screen bg-[#0A0A0C] text-white grid-bg"><Sidebar/><main className="lg:pl-64 pb-20"><div className="max-w-6xl mx-auto px-4 md:px-6 py-8">
    ...
    <div className="mt-8 rounded-3xl border border-white/10 bg-[#161616] p-6">
      ...
      {plans.length === 0 ? <div className="mt-5 rounded-2xl border border-dashed border-white/15 p-6 text-white/50">El catálogo todavía está vacío. La infraestructura de billing ya existe; falta definir y cargar los planes definitivos.</div> : <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4 mt-5">{plans.map((p)=><div key={p.id} className="rounded-2xl border border-white/10 bg-black/10 p-5"><div className="text-xs text-[#D4AF37] font-black uppercase tracking-widest">{p.audience_type}</div><h3 className="text-xl font-black mt-2">{p.name}</h3><p className="text-white/45 text-sm mt-2 min-h-10">{p.description || 'Plan DYNASTY'}</p><div className="mt-4 text-2xl font-black">{Number(p.price).toLocaleString('es-CO')} {p.currency_code}</div><div className="text-xs text-white/40 mt-1">{p.billing_interval}</div></div>)}</div>}
    </div>

    <section className="mt-8 rounded-3xl border border-[#D4AF37]/20 bg-[#D4AF37]/5 p-6"><h2 className="text-2xl font-black">Ciclo de acceso protegido</h2><div className="flex flex-wrap gap-2 mt-4 text-sm font-black">{['ACTIVO','VENCIDO','GRACIA','RESTRINGIDO','SUSPENDIDO'].map(x=><span key={x} className="rounded-full border border-white/10 bg-black/20 px-4 py-2">{x}</span>)}</div><p className="text-white/50 text-sm mt-4">La plataforma debe restringir herramientas por entitlements sin borrar la actividad histórica del club, coach u organizador.</p></section>
