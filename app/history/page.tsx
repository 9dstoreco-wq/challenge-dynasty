...
export default async function HistoryPage(){
 ...
 if(!user) return <main className="min-h-screen bg-[#0A0A0C] text-white grid place-items-center"><Link href="/login" className="text-[#D4AF37] font-black">Inicia sesión para ver tu historial</Link></main>
 ...
 return <div className="min-h-screen bg-[#0A0A0C] text-white grid-bg"><Sidebar/><main className="lg:pl-64 pb-20 lg:pb-0"><div className="max-w-5xl mx-auto px-4 md:px-6 py-8"><PageHero><div className="text-xs tracking-[.3em] text-[#D4AF37] font-black">DYNASTY HISTORY</div><h1 className="text-4xl md:text-6xl font-display font-black tracking-wide mt-2">Historial competitivo</h1><p className="text-white/45 mt-2">Cada partido confirmado deja rastro en tu historial.</p></PageHero><div className="space-y-3 mt-7">{(matches??[]).map((m)=>{const r=resultByMatch.get(m.id);const won=r?.winner_profile_id===user.id;const creator=profileById.get(relatedRow(m.challenge)?.creator_id);const resultData=r?.result_data??{};return <Link key={m.id} href={`/challenge/${relatedRow(m.challenge)?.id}`} className="block rounded-3xl border border-white/10 bg-[#161616] p-5 hover:border-gold-300/20"><div className="flex items-center gap-4"><div className={`h-14 w-14 rounded-2xl flex items-center justify-center font-black ${won?'bg-[#00E676]/10 text-[#00E676]':'bg-red-500/10 text-red-300'}`}>{won?'W':'L'}</div><div className="flex-1"><div className="text-xs text-white/35">{relatedRow(m.sport)?.icon} {relatedRow(m.sport)?.name} · {m.completed_at?new Date(m.completed_at).toLocaleDateString('es-CO'):'—'}</div><div className="font-black text-lg mt-1">{relatedRow(m.challenge)?.title??'Partido'}{creator&&creator.id!==user.id?<span className="text-white/45 font-medium"> · vs {creator.display_name}</span>:''}</div><div className="text-sm text-white/45 mt-1">{m.location_name??'Ubicación no indicada'}</div></div><div className="text-right"><div className="font-black text-xl">{resultData.score_set1||'—'} {resultData.score_set2?` · ${resultData.score_set2}`:''} {resultData.score_set3?` · ${resultData.score_set3}`:''}</div><div className={`text-xs font-black mt-1 ${won?'text-[#00E676]':'text-red-300'}`}>{won?'VICTORIA':'DERROTA'}</div></div></div></Link>})}{(matches??[]).length===0&&<div className="rounded-3xl border border-white/10 bg-[#161616] p-8 text-center text-white/45">Todavía no tienes partidos confirmados.</div>}</div></div></main><BottomNav/></div>
}
